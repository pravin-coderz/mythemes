...
/*Export START*/
if(isset($_REQUEST['export']) && $_REQUEST['export'] == 'reports_list'):
    
    /*
     * #Export (Currency Orders)
     */
    $file_name = 'applicantlist_'.date("M_d_Y_H_i").'.csv';
    $field_args = array(
        'name'        => 'NAME',
        'email'       => 'EMAIL',
        'telephone'   => 'TELEPHONE',
    ...
    // Read from Database.
    $query = "SELECT * FROM ".$wpdb->prefix."career ORDER BY `posted_date` desc";
...
class Applicant_List_Table extends WP_List_Table
{
    ...
    function prepare_items()
    {
        ...
        $query = "SELECT * FROM ". $wpdb->prefix ."career";
    ...
    {  ?>
        <br /><span><p>If you want to export the Contact Enquiries as a CSV file, Please click the export button below.</p><a href="?page=<?php echo $_REQUEST['page']?>&export=reports_list" id="export-reports_list" class="button" style="background-color:#0073aa; color:#fff;">Export</a></span>
        <?php
       $records = $this->items;
            ...
            foreach($records as $rec) {
                ...
                foreach ( $columns as $column_name => $column_display_name ) {
                    ...
                    switch ( $column_name ) {
                        ...
                        break;
                        case "col_filename": 
                            echo '<td '.$attributes.'><a href="'.stripslashes($rec->filename).'" download>'.stripslashes($rec->filename).'</a></td>';
                        break;
...
add_action('admin_menu', 'applicant_list');
...
function reportResult()
{
    $wp_list_table = new Applicant_List_Table();
